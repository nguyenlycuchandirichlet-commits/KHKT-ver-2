...
export default function AnalyticsDashboard({
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      label: 'Tỷ lệ từ phản biện',
      value: `${(criticalRatio * 100).toFixed(1)}%`,
      barValue: criticalRatio * 100,
      barMax: 100,
  ...
  const scoreBreakdown: { key: keyof Scores; label: string; icon: React.ReactNode; desc: string }[] = [
    ...
    },
    {
      ...
      desc: 'Tốc độ viết ổn định và ít lần xoá',
    },
    {
      key: 'independence',
      label: 'Độc lập',
  ...
  const handleExportCSV = () => {
    const rows = [
      ...
      ['Chỉ số lập luận', `${logicIndex}/5`],
      ['Tỷ lệ từ phản biện', `${(criticalRatio * 100).toFixed(1)}%`],
  ...
  return (
    <div className="min-h-screen px-4 pt-24 pb-16 sm:px-6">
      ...
      <div className="mx-auto max-w-6xl">
          ...
          {!isViolation && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-400 dark:text-slate-500">
                {result.rankPoints || 0} điểm xếp hạng
              </span>
            </div>
          )}
        </div>
        ...
        {/* KPI Scorecards */}
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 animate-fade-in-up">
          {kpiCards.map((kpi) => (
            <div
              key={kpi.label}
            ...
            >
              ...
              <p className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 sm:text-3xl">
                {kpi.value}
              </p>
              ...
              {kpi.barMax && (
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
                  <div
                    ...
                    style={{ width: `${Math.min((kpi.barValue / kpi.barMax) * 100, 100)}%` }}
                  />
                </div>
              )}
            </div>
          ...
          <div className="mb-8 animate-fade-in-up">
            <div className="mb-4 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Phân tích chi tiết — trực diện và xây dựng</p>
              </div>
            </div>
        ...
        <div className="mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-brand-900 to-slate-900 p-6 text-white shadow-lg animate-fade-in-up sm:p-8">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-white">Tổng hợp năng lực</h3>
          ...
          <button
            onClick={() => {
              const shareText = `Tôi vừa đạt ${overall}/100 điểm trong bài luyện phản biện! ${result.badges?.map((b: Badge) => b.emoji).join(' ') || ''}`;
              if (navigator.share) {
                navigator.share({ title: 'Kết quả luyện tập của tôi', text: shareText });
              } else {
...
function getEmotionalColors(scores: Scores, telemetry: { wpm: number; idleSeconds: number; tabViolations: number }) {
  ...
  if (scores.depth >= 70) colors.push({ color: 'from-orange-500 to-red-600', emoji: '🌋', label: 'Sâu sắc' });
  ...
  if (scores.speed >= 70) colors.push({ color: 'from-yellow-400 to-amber-500', emoji: '☄️', label: 'Nhanh nhạy' });
