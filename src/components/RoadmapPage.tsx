...
const ANTI_AI_ARGUMENTS = [
  'Khi học sinh dùng AI để viết, họ mất khả năng tự diễn đạt — đó là Semantic Drift.',
...
export default function RoadmapPage({
  ...
  const [progress, setProgress] = useState<Record<number, { status: DayStatus; completedAt: string | null; debateRounds: number; debateEvaluated: boolean }>>({});
  ...
  const loadProgress = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('*')
      .eq('user_id', user.id);
    const map: Record<number, { status: DayStatus; completedAt: string | null; debateRounds: number; debateEvaluated: boolean }> = {};
    (data || []).forEach((r: any) => {
      map[r.day] = {
        status: r.status,
    ...
    if (!map[1]) {
      map[1] = { status: 'available', completedAt: null, debateRounds: 0, debateEvaluated: false };
    }
    setProgress(map);
    setLoading(false);
  }, [user, profile]);
  ...
  const completeDay = async (day: number, debateRounds = 0, debateEvaluated = false) => {
    if (!user) return;
    triggerLavaBurst();
    ...
    <div className={`min-h-screen px-4 pt-24 pb-16 sm:px-6 ${screenShake ? 'animate-screen-shake' : ''}`}>
      ...
      {selectedDay && (
        <DayDetailModal
          ...
          onComplete={(debateRounds, debateEvaluated) =>
            completeDay(selectedDay.day, debateRounds, debateEvaluated)
          }
...
function DayDetailModal({
  ...
  day: RoadmapDay;
  progress?: { status: DayStatus; completedAt: string | null; debateRounds: number; debateEvaluated: boolean };
  onComplete: (debateRounds?: number, debateEvaluated?: boolean) => void;
  onClose: () => void;
  onNavigate: (page: Page) => void;
}) {
  ...
  const evaluateDebate = () => {
    ...
    setTimeout(() => {
      ...
      setDebateEval({
        ...
        total,
        strengths: wc > 50
          ? 'Lập luận có độ dài đủ để phát triển ý. Số lượt phản biện cho thấy không né tranh luận.'
          : 'Bước vào tranh luận thay vì né tránh — tín hiệu tốt.',
        weaknesses: debateRounds < 2
          ? 'Cần thêm lượt trao đổi để đào sâu luận điểm. 1-2 lượt chưa đủ bẻ gãy luận điểm đối lập.'
          : vocabScore < 50
            ? 'Từ vựng phản biện còn hạn chế — thiếu "luận điểm", "bằng chứng", "mâu thuẫn".'
            : 'Một số luận điểm chưa đủ cụ thể — cần dẫn chứng thực tế.',
        suggestion: 'Đặt câu hỏi "Tại sao?" cho chính luận điểm trước khi viết. Phát hiện lỗ hổng logic.',
      });
      setEvaluating(false);
    }, 1500);
  };
  ...
  const triggerShake = () => {
    setShake(true);
    setSpamToast('Định lừa AI à? Viết lại đi fen ơi, viết gì mà ngắn thế!');
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {isTemplate ? (
            <TemplateDismantlingStation onComplete={() => { setSubmitted(true); onComplete(); }} />
          ) : isSpeed ? (
            <SpeedRebuttalStation onComplete={() => { setSubmitted(true); onComplete(); }} />
          ) : isSummit ? (
            <SummitAssessmentStation onComplete={() => { setSubmitted(true); onComplete(); }} />
          ) : isDebate ? (
          ...
          {!isTemplate && !isSpeed && !isSummit && (
            <Button
              onClick={() => {
                if (isDebate) {
                  setSubmitted(true);
                  onComplete(debateRounds, !!debateEval);
                } else {
                  handleWritingSubmit();
                }
              }}
